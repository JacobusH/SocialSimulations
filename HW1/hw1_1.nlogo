...
to setup
  clear-all
  ; init global spot trackers
  set white-spots no-patches
  set blue-spots no-patches
  set orange-spots no-patches
  set is-debug false
  ...
  ask patches [
    ...
    set pcolor white
    ifelse random 100 < density [   ; set the occupancy density
      sprout 1 [
        ; 105 is the color number for "blue"
        ; 27 is the color number for "orange"
        set color one-of [105 27]
        if color = 105 [ set blue-spots (patch-set blue-spots cur-patch) ]
        if color = 27 [ set orange-spots (patch-set orange-spots cur-patch) ]
        set size 1
      ]
    ]
    [ set white-spots (patch-set white-spots cur-patch) ]
  ]
  if is-debug [
    print white-spots
    print orange-spots
...
to move-unhappy-turtles
  ask turtles with [ not happy? ]
    [ find-perfect-new-spot ]
  if is-debug [
    print count white-spots
  ]
end
...
; loop thru all patches. Find first empty 'perfect' one and move there
to find-perfect-new-spot
  let found-happy-spot false
  let cur-turtle self
  let old-patch patch 0 0 ; init old-patch
  ask cur-turtle [
    set old-patch patch xcor ycor
  ]
  ; go thru white-spots and find an acceptable spot
  ask white-spots [
    let new-spot self
    if found-happy-spot [ stop ]
    if not any? turtles-on new-spot [
      ask cur-turtle [
        move-to new-spot
        update-turtle cur-turtle
        if happy? [
          set found-happy-spot true
          ; update-white-spots
          move-white-spot old-patch new-spot
        ]
      ]
    ]
  ]
  if is-debug [
    if not found-happy-spot [ print "no happy spot" ]
  ]
end
...
; maintain the list of white-spots after a movement
to move-white-spot [ old-spot new-spot ]
  set white-spots (patch-set white-spots new-spot) ; add new patch
  set white-spots white-spots with [[old-spot] of myself != self] ; remove old patch
end

; maintain the list of white-spots after all turtles moved in a tick
to update-white-spots
  ; just loop thru the whole board again and get all the white patches
  set white-spots no-patches
  ask patches [
    let cur-patch self
    if not any? turtles-on cur-patch [
      set white-spots (patch-set white-spots cur-patch)
    ]
  ]
end
