...
turtles-own [
  ...
  social-network   ; the network of turtles this turtle can talk to
  open-neighbors   ; set of open spots next to the turtle
  happy-open-neighbors ; set of open spots next to a happy turtle
]
  ...
  ask patches [
    ...
    ifelse random 100 < density [   ; set the occupancy density
      sprout 1 [
        ...
        set size 1
        if color = 105 [ set turts-bl (turtle-set turts-bl self) ]
        if color = 27 [ set turts-orj (turtle-set turts-orj self) ]
      ]
    ]
    [ set white-spots (patch-set white-spots cur-patch) ]
  ]
  ; now from each colors' set, form a social network of the appropriate size
  ask turtles [
    let tmp-set no-turtles
    ifelse color = 105 [
      set tmp-set n-of net-size turts-bl
    ]
    [ set tmp-set n-of net-size turts-orj ]
    set social-network tmp-set
    if is-debug [ print social-network ]
  ]
  if is-debug [
    print white-spots
    print turts-orj
...
to move-unhappy-turtles
  ask turtles with [ not happy? ]
    [
      find-neighbor-spot
      ask turts-bl [ update-turtle-neighbors ]
      ask turts-orj [ update-turtle-neighbors ] ; can't ask all turtles here...
    ]
end
...
to find-neighbor-spot
  let cur-turtle self
  ask social-network [
    if any? happy-open-neighbors [
      let open-spot one-of happy-open-neighbors
      ask cur-turtle [ move-to open-spot]
    ]
  ]
end

to update-turtle-neighbors
  ; keep track of open neighbor spots
  set open-neighbors neighbors with [ not any? turtles-here ]
  ifelse happy? [
    set happy-open-neighbors neighbors with [not any? turtles-here]
  ]
  [ set happy-open-neighbors no-patches ]
end
