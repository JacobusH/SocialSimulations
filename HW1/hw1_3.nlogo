...
turtles-own [
  ...
  similar-pref     ; this turtle's simliarity prefernce for neighbors
]
...
to setup
  ...
  ; NOTE: 'random' creates a uniform distribution according to the docs
  ask turtles [
    set similar-pref ((random %-similar-wanted) + 1) ; NOTE: if %-similar-wanted is 10, will give values between 1 and 10 uniformly
  ]
  update-turtles
  update-globals
  reset-ticks
...
;    set happy? similar-nearby >= (%-similar-wanted * total-nearby / 100) ; NOTE: old
    set happy? similar-nearby >= (similar-pref  * total-nearby / 100) ; NOTE: old
